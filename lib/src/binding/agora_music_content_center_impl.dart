...
class MusicCollectionImpl implements MusicCollection {
  ...
  @override
  Future<Music> getMusic(int index) async {
    final apiType = '';
    final param = createParams({'index': index});
    final callApiResult =
        await apiCaller.callIrisApi(apiType, jsonEncode(param), buffers: null);
...
// }
  ...
  @override
  Future<void> getMusicCharts(String requestId) async {
    final apiType =
  ...
  @override
  Future<void> getMusicCollectionByMusicChartId(
  ...
  Future<void> getLyric(
      {required String requestId,
      required int songCode,
      int lyricType = 0}) async {
    final apiType =
        '${isOverrideClassName ? className : 'MusicContentCenter'}_getLyric';
    final param = createParams(
        {'requestId': requestId, 'songCode': songCode, 'LyricType': lyricType});
    final callApiResult =
        await apiCaller.callIrisApi(apiType, jsonEncode(param), buffers: null);
    if (callApiResult.irisReturnCode < 0) {
      throw AgoraRtcException(code: callApiResult.irisReturnCode);
